...
class FlashSale{
    async addNewFlashSale(payload){
        const data = [payload];
        let view = model.generalFlashSale();
        ...
        view = data.reduce((accumulator, value) => {
            ...
            if(!validate.isEmpty(value.percentage)){accumulator.percentage = value.percentage;}
            if(!validate.isEmpty(value.priceAfter)){accumulator.priceAfter = value.priceAfter;}
            if(!validate.isEmpty(value.category_id)){accumulator.category_id = value.category_id;}
            return accumulator;
        }, view);

        const document = view;
        const result = await command.insertOneFlashSale(document);

        return result;
    }

    async updateFlashSale(params, payload){
        const data = [payload];
        let view = model.generalFlashSale();
        ...
        const document = view;
        const result = await command.updateOneFlashSale(params, document);
        return result;
    }
